...
@section('content')
    ...
    {{-- Hero --}}
    <section class="relative overflow-hidden pt-10" style="background:linear-gradient(180deg, #ffffff 0%, #fafafa 55%, #E8F4FC 100%)">
        <div class="relative mx-auto max-w-[1400px] px-5 sm:px-8 lg:pl-28 pt-16 pb-20 grid lg:grid-cols-12 gap-6 sm:gap-12 items-center">
            <div class="lg:col-span-7">
                ...
                <div class="inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-[9px] sm:text-[11px] tracking-[0.18em] sm:tracking-[0.22em] uppercase whitespace-nowrap" style="border-color:rgba(0,0,0,0.18);color:#000000">
                    <span class="w-1.5 h-1.5 rounded-full" style="background:#4ECDC4"></span>{{ $settings['badge'] }}
                </div>
                @endif
                <h1 class="mt-6 text-[44px] sm:text-[60px] lg:text-[76px] leading-[0.95] tracking-tight font-semibold">
                    {{ $settings['title_line1'] ?? 'Build, scale,' }} <br/>
                    <span class="italic font-normal" style="color:#4A69BD">{{ $settings['title_highlight'] ?? 'operate' }}</span> — as one.
                </h1>
                <p class="mt-8 max-w-[560px] text-[17px] leading-[1.6]" style="color:#2b2b2b">{{ $settings['description'] ?? '' }}</p>
                @if($heroStats->count())
                <div class="mt-10 grid grid-cols-3 gap-6 max-w-[560px]">
                    @foreach($heroStats as $stat)
                    <div>
                        <div class="text-[28px] font-semibold leading-none" style="color:#000000">{{ $stat->value }}</div>
                        <div class="mt-2 text-[11px] tracking-[0.2em] uppercase" style="color:#6b6b6b">{{ $stat->label }}</div>
                    </div>
                    @endforeach
                </div>
                @endif
                <div class="mt-10 flex flex-wrap gap-3">
                    <a href="{{ url('/strategy-session') }}" class="lg-btn rounded-full border px-7 py-4 text-[14px] hover:opacity-90 transition">{{ $settings['cta_primary'] ?? 'Book A Strategy Session →' }}</a>
                    <a href="{{ url('/portfolio') }}" style="border-color:rgba(0,0,0,0.2);color:#000000" class="rounded-full border px-7 py-4 text-[14px] hover:opacity-90 transition">{{ $settings['cta_secondary'] ?? 'See Our Work' }}</a>
                </div>
            </div>
            <div class="lg:col-span-5 relative">
                <div class="relative rounded-[28px] overflow-hidden border" style="border-color:rgba(0,0,0,0.12);box-shadow:0 30px 80px -30px rgba(0,0,0,0.35)">
